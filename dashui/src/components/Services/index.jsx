...
class Services extends React.Component
{
    ...
    async get()
    {
      ...
      let data = await axios.get("admin/services")
    ...
    {
      let data = await axios.get("admin/services/"+item.id)
        .then(function(response) {
        return response.data.data
      }).catch(function(error) {
              ...
              return (
                  <Fragment>
                      <Link
                          ...
                          to={`${process.env.PUBLIC_URL}/dashboard/setting/services/edit/${record.id}`}
   ...
   handleDelete = () => {
    ...
    if (window.confirm(`${i18next.t('Aresuredelete')}:\r ${this.state.selectedRows.map(r => r.name)}?`)) {
      ...
      let deleteCategory = axios.post("admin/services/1", {
  ...
  render()
  {
    ...
    return(
      <Fragment>
        <Breadcrumb parent={i18next.t('setting')} title={i18next.t('services')}/>
        <Container fluid={true}>
                <Row>
                    ...
                    <Col sm="12">
                        <Card>
                            <CardHeader>
                                 <Link className="btn btn-primary active" to={`${process.env.PUBLIC_URL}/dashboard/setting/services/create`}>
                                     {i18next.t('createnew')}
                                  </Link>
                            </CardHeader>
                            <CardBody>
