...
class EditCareer extends React.Component
{
  constructor()
    {
      super()
    ...
    async getItem()
    {
      let data = await axios.get("admin/careers/"+this.props.match.params.id)
    ...
    handleChange=event=>
  {
    ...
    onSubmit = () => {
      ...
      formData.append('descriptionen', this.state.descen)
        ...
        axios.post("admin/careers/"+this.props.match.params.id, 
                   formData,config)
            .then(function (response) {
    ...
    render()
  {
    if (this.state.toDashboard === 1) {
      return <Redirect to={`${process.env.PUBLIC_URL}/dashboard/setting/careers`} />
    }
    ...
    return(
      <Fragment>
          <Breadcrumb parent={i18next.t('update')} title={i18next.t('careers')}/>
