...
class EditNews extends React.Component
{
  constructor()
    ...
    async getItem()
    {
      let data = await axios.get("admin/news/"+this.props.match.params.id)
    ...
    handleChange=event=>
  {
    ...
    onSubmit = () => {
      ...
      formData.append('descriptionen', this.state.descen)
        ...
        axios.post("admin/news/"+this.props.match.params.id, 
                   formData,config)
            .then(function (response) {
    ...
    render()
  {
    if (this.state.toDashboard === 1) {
      return <Redirect to={`${process.env.PUBLIC_URL}/dashboard/setting/news`} />
    }
    ...
    return(
      <Fragment>
          <Breadcrumb parent={i18next.t('update')} title={i18next.t('news')}/>
