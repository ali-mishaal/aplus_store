...
class EditInformation extends React.Component
{
  constructor()
    ...
    async getItem()
    {
      let data = await axios.get("admin/informations/"+this.props.match.params.id)
    ...
    handleChange=event=>
  {
    ...
    onSubmit = () => {
      ...
      formData.append('descriptionen', this.state.descen)
        ...
        axios.post("admin/informations/"+this.props.match.params.id, 
                   formData,config)
            .then(function (response) {
    ...
    render()
  {
    ...
    return(
      <Fragment>
          <Breadcrumb parent={i18next.t('update')} title={i18next.t('informations')}/>
